...
float pitchkp =50  , pitchkd = 0 , pitchki =0;
float rollkp =50  , rollkd = 0 , rollki =0;

int motorpin , rudderpin , elevatorpin , leftaileronpin , rightaileronpin=11 ;
...
//  Serial.println("");
  ...
  if(!autocondition){
    ...
    int pitchdelta=pitchpid(0,ypr[1]);
    int rolldelta=rollpid(0,ypr[2]);
    Serial.println(pitchdelta);
    if(pitchdelta>85||pitchdelta<-85)pitchdelta=0;
    if(rolldelta>85||rolldelta<-85)rolldelta=0;
    
    
    int leftoutput=90+rolldelta+pitchdelta;
    int rightoutput=90+rolldelta-pitchdelta;
    
    
    Serial.print(leftoutput);
    Serial.print("  ");
    Serial.println(rightoutput);
    leftaileron.write(leftoutput);
    rightaileron.write(rightoutput);
  }
