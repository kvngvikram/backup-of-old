! lax scheme is stable but will damp 
! lax scheme will not damp if Courant number = 1 
program adv
        ...
        do j = 1,N
                ...
                Un1(1) = (U(1+1)+U(M))/2 - (U(1+1)-U(M))*r/2
                write(fn,*) x(1) , Un1(1)
                do i = 2,M-1
                       Un1(i) = (U(i+1)+U(i-1))/2 - (U(i+1)-U(i-1))*r/2
                       write(fn,*) x(i),Un1(i)
                end do
                Un1(M) = (U(M-1)+U(1))/2 - (U(1)-U(M-1))*r/2
                write(fn,*) x(M),Un1(M)
                close(fn)
                U = Un1
                call system('gnuplot -p gnuplot.plt')
